...
def normalize_extraction_fields(text: str) -> str:
    """Normalize Markdown on protocol field lines, not arbitrary prose."""
    field = re.compile(r'^\s*(?:#{1,6}\s*)?(?:-\s*)?(?:\*\*)?(?:ENTITY|Description|Relationships|Source|Target)(?:\*\*)?\s*:', re.I)
    lines = []
    for line in text.splitlines():
        line = line.replace("**", "") if field.match(line) else line
        # AGEA sometimes puts a negative annotation in a Source slot, with no
        # Target. It is neither a real entity nor a truncated positive record.
        if re.match(r'^\s*(?:-\s*)?Source\s*:\s*\((?:No (?:direct |supported )?relationships?\b[^\n]*|None(?: listed[^\n]*)?)\)\s*$', line, re.I):
            line = "No supported relationships reported."
        lines.append(line)
    return "\n".join(lines)


def extraction_response_diagnostics(text: str, node_count: int, edge_count: int) -> dict:
    """Distinguish explicit emptiness, format loss, and parsed records.

    This is a syntax audit, not evidence verification or a truth judgment.
    """
    actual = normalize_extraction_fields(extract_actual_llm_response(text)).strip()
    explicit_empty = actual == "NO_SUPPORTED_RECORDS"
    source_seen = target_seen = False
    ambiguous_targets = source_fields = incomplete_sources = 0
    for line in actual.splitlines():
        # The adapter also supports a complete comma-separated inline format.
        if re.search(r'Source\s*:.*?,\s*Target\s*:.*?,\s*Description\s*:', line, re.I):
            continue
        if re.search(r'Source\s*:.*?(?:→|->)\s*Target\s*:.*?(?:—|--|\s-\s)', line, re.I):
            continue
        match = re.match(r'^\s*(?:#{1,6}\s*)?(?:-\s*)?(ENTITY|Source|Target)\s*:', line, re.I)
        if not match:
            continue
        field = match.group(1).lower()
        if field == "entity":
            incomplete_sources += int(source_seen and not target_seen)
            source_seen = target_seen = False
        elif field == "source":
            incomplete_sources += int(source_seen and not target_seen)
            source_fields += 1
            source_seen, target_seen = True, False
        else:
            ambiguous_targets += int(not source_seen or target_seen)
            target_seen = True
    incomplete_sources += int(source_seen and not target_seen)
    status = "parsed" if node_count or edge_count else "unstructured_empty"
    if explicit_empty:
        status = "explicit_empty"
    conflicting_empty = "NO_SUPPORTED_RECORDS" in actual and not explicit_empty
    if ambiguous_targets or incomplete_sources or conflicting_empty or (source_fields and not edge_count):
        status = "format_error"
    return {"parse_status": status, "source_fields": source_fields,
            "ambiguous_target_fields": ambiguous_targets,
            "incomplete_source_fields": incomplete_sources,
            "conflicting_empty_marker": conflicting_empty}
...
def parse_llm_response_for_graph_items(
    ...
    *,
    ...
    llm_response = normalize_extraction_fields(llm_response)
    if extract_actual_llm_response(llm_response).strip() == "NO_SUPPORTED_RECORDS":
        return [], []
...
def parse_entities_and_relationships_improved(text: str) -> Dict[str, List[Dict[str, str]]]:
    ...
    for name, start, end in entity_spans:
        ...
        if name and (name not in nodes or (not nodes[name] and desc)):
            nodes[name] = desc
...
def parse_structured_relationships(llm_response: str) -> List[Dict[str, Any]]:
    ...
    # Read explicit Source/Target/Description records even when the model uses
    # a plain heading (e.g. '### HEYWOOD') rather than 'ENTITY: HEYWOOD'.
    # Heading boundaries keep one entity's trailing text out of the next block.
    for block in re.split(r'(?mi)^(?=[ \t]*(?:#{1,6}\s+|ENTITY\s*:))', llm_response):
        for rel in _parse_relationships_block_improved(block):
            description = re.sub(r'\[Data:.*?\]', '', rel['description']).strip()
            if rel['src'] and rel['target'] and rel['src'] != rel['target'] and len(description) > 10:
                relationships.append({'source': rel['src'], 'target': rel['target'],
                    'rel': 'related_to', 'weight': 1.0, 'description': description})
    ...
    for raw in lines:
        ...
        if m_tgt:
            if cur["target"] is not None:
                # Never overwrite a complete relation with an orphan Target.
                # The syntax audit flags this response for caller inspection.
                _flush()
                cur = {"src": None, "target": None, "description": []}
